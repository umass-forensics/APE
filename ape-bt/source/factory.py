...
#from experiments.Scenario0 import Scenario0
...
class BTClientFactory(protocol.ClientFactory, ConnectionManagerBase):
    ...
    def __init__(self, btm):
        ...
        if btm.app.bind_ip:
            self.bind_addr = (btm.app.bind_ip, 0)
        else:
            self.bind_addr = None
    ...
    def conncectPeers(self, peers):
        for addr, port in peers:
            print 'Info: attempting to connect to peer at {0}:{1}'.format(addr,port)
            print 'Using bindaddr %s' % str(self.bind_addr)
            reactor.connectTCP(addr, port, self, bindAddress=self.bind_addr)
            yield sleep(0)
    ...
    def clientConnectionFailed(self, connector, reason):
        print 'Info: Connection Failed, {0} : {1}'.format(
                self.getPeerAddr(connector),
                reason)
        self.connectRetry(connector)

    def clientConnectionLost(self, connector, reason):
        print 'Info: Connection Lost, {0} : {1}'.format(
                self.getPeerAddr(connector),
                reason)
        #RJW: Don't try to reconnect since the connection may close cleanly
        #when the target finishes downloading
        #self.connectRetry(connector)
    ...
    def connectRetry(self, connector):
        ...
        if retry > 50:
            ...
            yield sleep(5)
            print 'Info: Retrying connection to {0}'.format(addr)
            connector.connect()
            retry += 1
...
class BTServerFactory (protocol.ServerFactory, ConnectionManagerBase):
    ...
    #RJW: It does not seem like this protocol field is used.
    #protocol = None
...
class BTServerFactories (protocol.ServerFactory):
    ...
    def __init__(self, listen_port=6881, exp_num=0):
        self.maps = {}
